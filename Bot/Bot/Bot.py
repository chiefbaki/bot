...
class VkBot:
    ...
    def _show_5days_weather(self, user_id: int) -> None:
        ...
        response = requests.get("http://api.openweathermap.org/data/2.5/forecast",
                                params={'q': 'moscow,ru', 'units': 'metric',
                                        'APPID': '3051c8da538a2e104685783dae4796f4', 'lang': 'ru'})
        info = response.json()
        for i in range (0, 5):
            img_name = info['list'][2]['weather']['icon']
            im = Image.open(urlopen('https://openweathermap.org/img/wn/{}@4x.png'.format(img_name)))
            img.paste(im, (95, 20), im.convert('RGBA'))
            status = info['weather'][i]['description'].capitalize()
            temp = str(int(info['main']['temp_min'])) + ' - ' + str(int(info['main']['temp_max']))
            pressure = str(int(float(info['main']['pressure']) / 1.33))
            humidity = str(info['main']['humidity'])
            wind_speed = info['wind']['speed']
            ...
            weather = '{}, температура: {}°C\nДавление: {} мм рт. сб., влажность: {}%\nВетер: {}, {} м/с, {}'.format(status,
                    temp,pressure,humidity,wind_slug,wind_speed,wind_deg_slug)
            idraw.text((10, 185), weather, font=font, fill="white")
            img.save('data/weather_card.png')

            photo = upload.photo_messages('data/weather_card.png')[i]
            attachments.append("photo{}_{}".format(photo["owner_id"], photo["id"]))

        self._send_message_with_attachments(user_id=user_id, text='на 5 дней', attachments=attachments)
